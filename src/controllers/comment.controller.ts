...
export async function updateComment(req: Request, res: Response) {
  try {
    ...
    if (!commentId || !description || !id) {
      return res.status(400).json({ msg: "Missing required fields" });
    }

    const isCommentExist = await pool.query(
      `SELECT * FROM comments WHERE id = $1 AND commenter_id = $2`,
      [commentId, id]
    );
    ...
    const isCommentUpdated = await pool.query(
      `UPDATE comments SET description = $1 WHERE id = $2 AND commenter_id = $3`,
...
export async function deleteComment(req: Request, res: Response) {
  try {
    ...
    const isCommentDeleted = await pool.query(
      `DELETE FROM comments WHERE id = $1 AND commenter_id = $2`,
      [commentId, id]
    );
